...
amount_to_sample = floor(sum(model_data$hospital_expire_flag == 1) * 0.8)


train <- model_data %>% group_by(hospital_expire_flag) %>% sample_n(size = amount_to_sample) %>% ungroup()
test <- model_data %>% anti_join(train, by="id") %>% select(-id)
...
# 
# train_expire <- sample_n(train_expire, (nrow(train_survive) * 0.7), replace = FALSE)
# 
# 
...
modelTuning <- tune(svm,
              ...
              data= train, 
              probability=TRUE,
              ranges = list(cost = c(100,200,300,400,500,750,1000)), 
              scale = TRUE)
