...
# oversample the train data set for hospital_expire_flag = 1 since it makes up ~ 16% of the data set

train_survive <- train[train$hospital_expire_flag == 0] %>% select(-id)
train_expire <- train[train$hospital_expire_flag == 1] %>% select(-id)

train_expire_oversample <- sample_n(train_expire, (nrow(train_survive) * 0.7), replace = TRUE)
train_oversampled <- bind_rows(train_survive, train_expire_oversample)

# build the model
model <- e1071::svm(hospital_expire_flag ~ ., train_oversampled, kernel = "linear", cost = 100, probability = TRUE, scale = TRUE)
challenger <- e1071::svm(hospital_expire_flag ~ age_range +
  heart_rate +
  sysbp_mean +
  resprate_mean +
  tempc_mean +
  platelet_min +
  is_male, train_oversampled, kernel = "linear", cost = 100, probability = TRUE, scale = TRUE)
...
model_predictions <- ROCR::prediction(model_probabilities, test$hospital_expire_flag)
...
challenger_predictions <- ROCR::prediction(challenger_probabilities, test$hospital_expire_flag)
