...
# setup the search grid with a collection of potential values for gamma and cost
# each will increment by a step of 0.01. gammas are between 0.01 and 1 inclusive and
# cost will be 0.01 through 1000 in steps that are powers of 10
potentialGammas <- seq(from = 0.01, to = 1, by = 0.01)
potentialCosts <- 0.01 * 10^(0:5)
searchGrid <- expand.grid(gamma = potentialGammas, cost = potentialCosts)
...
cParallelCluster <- makeCluster(detectCores() - 1)
clusterSetupRNG(cParallelCluster, seed = randomSeed)
registerDoSNOW(cParallelCluster)
...
cat(
  ...
  paste("cost  =", bestTuneResult$cost),
  paste('-----------------------------'),
